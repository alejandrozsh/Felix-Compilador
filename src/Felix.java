...
<<<<<<< HEAD
    // [CAMBIO 4] Lista para acumular errores sintácticos sin detener el análisis
...
<<<<<<< HEAD
    // [CAMBIO 4] Recuperación de errores en modo pánico.
    // Usa getToken(1) (peek) para NO consumir tokens de sincronización.
    // Se detiene al encontrar:
    //   - ";" → lo consume (fin de la sentencia errónea)
    //   - Keyword de inicio de sentencia → NO lo consume (el parser reintenta)
    //   - Keyword de fin de bloque / ELSE / CASE → NO lo consume (la regla padre lo necesita)
    //   - EOF → NO lo consume
    private void recoverFromError() {
        try {
            Token t = getToken(1); // peek sin consumir
            while (t.kind != SEMICOLON && t.kind != EOF
                   // Keywords de inicio de sentencia (nueva sentencia por delante)
                   && t.kind != SET && t.kind != WRITE && t.kind != READ
                   && t.kind != IF && t.kind != WHILE && t.kind != FOR
                   && t.kind != SWITCH && t.kind != NEW
                   // Keywords de fin de bloque (la regla padre los necesita)
                   && t.kind != ENDIF && t.kind != ENDWHILE
                   && t.kind != ENDFOR && t.kind != ENDSWITCH
                   && t.kind != ELSE && t.kind != CASE && t.kind != DEFAULT_CASE
                   && t.kind != DO && t.kind != THEN) {
                getNextToken(); // consumir el token actual (basura)
                t = getToken(1); // peek al siguiente
            }
            // Si paramos en ";" lo consumimos (cierra la sentencia errónea)
            if (t.kind == SEMICOLON) {
                getNextToken();
            }
            // Para keywords y EOF NO consumimos: el parser los verá al reintentar
        } catch (Exception e) { /* red de seguridad */ }
        // Limpiar la pila semántica para evitar errores en cascada
        // por nodos residuales de expresiones parcialmente parseadas
        semanticStack.clear();
    }

    // [CAMBIO 4] Formatea un ParseException para mostrarlo como error sintáctico
    private String formatSyntaxError(ParseException e) {
        int line = 0, col = 0;
        ...
        if (e.currentToken != null && e.currentToken.next != null) {
            ...
            detail = traducirExpectedTokens(e);
        }
        return "\n==================================================\n" +
               "ERROR SINT\u00c1CTICO\n\n" +
               "L\u00ednea: " + line + "\n" +
...
<<<<<<< HEAD
        // [CAMBIO 4] Los errores sintácticos ahora se acumulan en syntaxErrors
        // gracias a la recuperación en Statement(). El catch aquí es solo seguridad.
        Felix parser = new Felix(inputStream);
        try {
            parser.Start();
        } catch (ParseException e) {
            // Error sintáctico fuera de Statement() (ej. estructura de nivel superior)
            parser.syntaxErrors.add(parser.formatSyntaxError(e));
        } catch (TokenMgrError e) {
            // Error léxico fatal (no debería llegar aquí, pero por seguridad)
            parser.lexicErrors.add("Car\u00e1cter no reconocido (error l\u00e9xico fatal).");
        }

        // Determinar si hay CUALQUIER tipo de error antes de escribir el archivo
        boolean hayErrores = !parser.syntaxErrors.isEmpty()
...
<<<<<<< HEAD
        if (!parser.syntaxErrors.isEmpty()) {
            ...
            System.out.println("==================================================");
            for (String se : parser.syntaxErrors) {
...
// [CAMBIO 4] Statement() con recuperación de errores: si una sentencia falla,
// se registra el error y se avanza hasta el siguiente punto de sincronización
// para continuar analizando el resto del código.
...
// Filtrar errores en cascada: si el token problemático es un ERROR léxico
        // (tilde, carácter inválido, etc.) ya fue reportado en lexicErrors;
        // no agregamos un error sintáctico duplicado.
        if (e.currentToken != null && e.currentToken.next != null
            && e.currentToken.next.kind == ERROR) {
            // Solo recuperar, sin registrar error sintáctico duplicado
        } else {
            syntaxErrors.add(formatSyntaxError(e));
        }
        recoverFromError();
    }
}
...
<<<<<<< HEAD
  ...
  final public void EscribirTexto() throws ParseException {ExprNode tree;
    Token t;
...
<<<<<<< HEAD
  ...
  final public void LeerVariable() throws ParseException {Token id;
...
// ============================================================
<<<<<<< HEAD
  final public void Condicional() throws ParseException {ExprNode tree;
